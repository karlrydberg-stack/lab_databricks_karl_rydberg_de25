...
def cleaned_marathos():
    ...
    df = df.withColumn("event_start_date", to_date(col("temp_start"), "dd.MM.yyyy"))
    df = df.drop("temp_start", "event_dates")
    
    df = df.withColumn(
        "athlete_country", 
        when(upper(col("athlete_country")) == "XXX", None)
        .when(upper(col("athlete_country")) == "SVE", "SWE")
        .otherwise(upper(col("athlete_country"))),
    )
    ...
    df = df.withColumn(
        ...
        .otherwise(col("athlete_year_of_birth").cast("integer")),
    )
    ...
    df = df.withColumn(
        ...
        when(
            ...
            (col("athlete_average_speed") > 30.0), 
            None,
        )
